...
def kplot(df, position = None, wealth = None):
    ...
    if position is not None:
        ...
        for i in range(signal_count):
            fig.add_trace(go.Scatter(
                ...
                yaxis  = 'y2',
                mode   = 'lines+markers',
                marker = dict(color = UP_COLOR if signal_side[i][0] > 0 else DOWN_COLOR),
                line   = dict(color = UP_COLOR if signal_side[i][0] > 0 else DOWN_COLOR),
                showlegend = False,
            ))
