...
class ArrayManager(object):
    ...
    def __init__(self, 
        ...
        self.dt_offset: int = dt_offset
        self.last_dt = datetime.datetime(2000, 1, 1, 14, 59)

        self.transforms = [getattr(x, 'vnpy_transform')() for x in strategies]
        self.transform_func = [x[0] for x in self.transforms]
        self.min_size = max([x[1].get('min_size', 0) for x in self.transforms]) + 1
        self.daily_size = max([x[1].get('daily_size', 0) for x in self.transforms]) + 2

        self.open_array: np.ndarray = np.zeros(self.min_size)
        self.high_array: np.ndarray = np.zeros(self.min_size)
        self.low_array: np.ndarray = np.zeros(self.min_size)
        self.close_array: np.ndarray = np.zeros(self.min_size)
        self.volume_array: np.ndarray = np.zeros(self.min_size)
        self.open_interest_array: np.ndarray = np.zeros(self.min_size)

        self.daily_open_array: np.ndarray = np.zeros(self.daily_size)
        self.daily_high_array: np.ndarray = np.zeros(self.daily_size)
        self.daily_low_array: np.ndarray = np.zeros(self.daily_size)
        self.daily_close_array: np.ndarray = np.zeros(self.daily_size)
        self.daily_volume_array: np.ndarray = np.zeros(self.daily_size)
        self.daily_open_interest_array: np.ndarray = np.zeros(self.daily_size)
    ...
    def is_next_day(self):
        ...
        # elif current_time == datetime.time(9, 0):
        #     current_date = current_dt.date()
        #     # last_date    = last_dt.date()

        #     if   (current_date - last_dt).days == 1:
        #         return False
        #     elif 
        #     if (current_date - last_dt).days > 3
        #     if (current_dt.weekday() == 0) and ((current_date - last_dt).days == 3):
        #         return False
        #     else:
        #         return True

        if self.last_dt.time() == datetime.time(14, 59):
            return True
    ...
    def update_bar(self, bar: BarData) -> None:
        ...
        if self.daily_size > 0:
            # if (self.last_dt != (bar.datetime + datetime.timedelta(minutes = self.dt_offset)).date()) and (bar.datetime.time() > datetime.time(20, 0)):
            if self.is_next_day():
                # print(bar.datetime)
                self.daily_count += 1
                ...
                # self.last_dt = (bar.datetime + datetime.timedelta(minutes = self.dt_offset)).date()
